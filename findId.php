...
if ($argc <= 1) exit("Error: Missing ID to search for!");

$idToFind = intval($argv[1]);
print("\tLooking for ID $idToFind in all Destiny2 tables...\n");
...
{
	$result = $db->query("SELECT COUNT(*) as c FROM `$table` WHERE id='$idToFind';");
	
	if ($result === false) 
	{
		$result = $db->query("SELECT COUNT(*) as c FROM `$table` WHERE `key`='$idToFind';");
